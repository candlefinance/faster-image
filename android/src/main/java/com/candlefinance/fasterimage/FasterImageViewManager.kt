...
class FasterImageViewManager : SimpleViewManager<AppCompatImageView>() {
  ...
  fun setImageSource(view: AppCompatImageView, options: ReadableMap) {
    ...
    val ignoreQueryParamsForCacheKey =
      if (options.hasKey("ignoreQueryParamsForCacheKey")) options.getBoolean("ignoreQueryParamsForCacheKey") else false
  ...
  private fun makeThumbHash(view: AppCompatImageView, hash: String): Drawable {
    ...
    val bitmap =
      Bitmap.createBitmap(intArray, thumbHash.width, thumbHash.height, Bitmap.Config.ARGB_8888)
